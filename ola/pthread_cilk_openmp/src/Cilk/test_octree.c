...
int main(int argc, char** argv){
  ...
  if (argc != 7) { // Check if the command line arguments are correct 
    printf("Usage: %s N dist pop rep P\n"
	   ...
	   "T    : number of threads(MAXIMUM NUMBER 9999.\n", argv[0]);
    return (1);
  }
  ...
  num_threads=(argv[6]);//number of threads
  //printf("%s",c);
  __cilkrts_end_cilk();//klinw tn cilk gia na orisw to number of workers
  __cilkrts_set_param("nworkers",num_threads);
  __cilkrts_init();// kai kanw initialize pali
